...
export const postData = async({route , token , data ,formData})=>{
    const Data = formData ? data : JSON.stringify(data)
    // console.log(Data )

    return axios.post(`${BASE_END_POINT}/${route}`, Data , {
        headers: {
...
const usePostData = () => {
    ...
    const postdata = async({route , data , formData})=>{
        console_log({route , data , token , formData})
        try {

            setLoading(true)
            const res = await postData({route , token , data , formData})
            console_log('resresresresres',{res} , )
